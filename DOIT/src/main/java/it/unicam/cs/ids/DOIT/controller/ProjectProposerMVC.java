...
public class ProjectProposerMVC {
    ...
    public Set<PartecipationRequest<ProgramManagerRole>> listPR(Long idUser, Long tokenUser, Long idProject) {
        ...
        return userHandler.getUser(user, tokenUser).getRolesHandler().getProjectProposerRole().getPartecipationRequestsByTeam(project);
    }
    ...
    public void removePR(Long idUser, Long tokenUser, Long idProgramManager, Long idProject, String reason) {
        User user = userRepository.findById(idUser).get();
        Project project = projectRepository.findById(idProject).get();
        User userPM = userRepository.findById(idProgramManager).get();
        ProgramManagerRole programManagerRole = userPM.getRolesHandler().getProgramManagerRole();
        userHandler.getUser(user, tokenUser).getRolesHandler().getProjectProposerRole().removePR(programManagerRole, project, reason);
        userRepository.save(user);
        userRepository.save(userPM);
    }
