...
class ProjectProposerRoleTest {
    ...
    void testCreateProject() {
        try {
            assertThrows(IllegalArgumentException.class,
                    () -> {
                        user2.getRole(ProjectProposerRole.class).createProject(1, "SO", "sistema operativo", category2, factory);
                    }, "L'utente non presenta la categoria: [INFORMATICA]");
            Project project = user2.getRole(ProjectProposerRole.class).createProject(1, "Campo da calcio", "calcio a 5", category1, factory);
    ...
    void testFindProgramManagerList() {
        try {
            Set<User> userSet = resourceHandler.search(User.class, user2.getRole(ProjectProposerRole.class).findProgramManagerList(category1));
